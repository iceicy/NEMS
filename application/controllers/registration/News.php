...
class News extends NEMs_Controller
{
    ...
    public function form_edit()
    {
        ...
        $picture = 'default_img.png'; //$this->input->post('picture');
...
//=====================================================
        ...
        $data['form'] = $this->form->get_by_key($news_id);
        //$data['form'] = array_shift($qu_form);
        $this->output('registration/news/form', $data);
        // redirect("registration/news/form");
    }
    public function list($news_id = null)
    {
        if (!empty($news_id)) {
            $data['form'] = $this->form->get_by_key($news_id);
            $this->output('registration/news/form', $data);
        } else {
            $data['table_news_list'] = $this->form->get_all_news();
            $this->output('registration/news/list', $data);
        }
    }

    public function delete_news($news_id = null)
    {
        $this->form->delete_by_key($news_id);
        $data['table_news_list'] = $this->form->get_all_news();
        $this->output('registration/news/list', $data);
    }
